...
pub fn convert_file_to_markdown(input_file: &Path, output_folder: &Path) -> io::Result<()> {
    ...
    if extension == "md" || extension == "markdown" {
        println!("Skipping Markdown file: {}", input_file.display());
        return Ok(());
    }
    ...
    // Serialize the meta data into YAML
    let yaml = serde_yaml::to_string(&meta)
        .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("YAML serialization error: {}", e)))?;
    ...
    // Insert exactly one blank line after front matter
    writeln!(md_file)?;

    // Open the code block
    if lang.is_empty() {
        // Unrecognized extension => plain fenced code block
        writeln!(md_file, "```")?;
    } else {
    ...
    println!("Converted {} -> {}", input_file.display(), md_output_path.display());
    Ok(())
}
        ...
        if path.is_dir() {
            ...
            if let Some(ext) = path.extension().and_then(|e| e.to_str()) {
                if ext.eq_ignore_ascii_case("md") || ext.eq_ignore_ascii_case("markdown") {
                    println!("Skipping Markdown file: {}", path.display());
                    continue;
                }
            }
