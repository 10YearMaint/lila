...
/// is inlined. For source files, the inlined content is wrapped in a fenced code block with
/// appropriate language and a ".cb-code" class.
fn inline_placeholders_in_file(file_path: &Path) -> io::Result<()> {
    ...
    let new_content = re.replace_all(&content, |caps: &regex::Captures| {
        ...
        if let Some((file_name, identifier)) = referenced.split_once(':') {
            ...
            if ref_path.exists() {
                if let Ok(Some(def)) = extract_definition_from_file(&ref_path, identifier) {
                    ...
                    if let Some(lang) = infer_language_from_extension(&ext) {
                        // Wrap the extracted definition in a fenced code block with language and class.
                        return format!("\n\n```{{.{} .cb-code}}\n{}\n```", lang, def);
                    } else {
                        return format!("\n\n```\n{}\n```", def);
                    }
            ...
            // If file not found or extraction fails, return the original placeholder.
            return caps.get(0).unwrap().as_str().to_string();
        } else {
            // No identifier provided, so include the entire file.
            ...
            if ref_path.exists() {
                if let Ok(file_content) = fs::read_to_string(&ref_path) {
                    let ext = Path::new(referenced)
                        ...
                        .to_lowercase();
                    if let Some(lang) = infer_language_from_extension(&ext) {
                        // Wrap the entire file content in a fenced code block.
                        return format!("\n\n```{{.{} .cb-code}}\n{}\n```", lang, file_content);
                    } else {
                        // For non-source files (or ones without a recognized extension), inline as-is.
                        return file_content;
                    }
                }
            }
            // If file not found, return the original placeholder.
            return caps.get(0).unwrap().as_str().to_string();
        }
    });
